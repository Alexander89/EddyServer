...
void SHA1::Transform(uint* pState, const unsigned char* pBuffer){
	uint a = pState[0], b = pState[1], c = pState[2], d = pState[3], e = pState[4];
...
void SHA1::update(const unsigned char* pbData, unsigned int uLen) {
...
void SHA1::final() {
	...
	for(i = 0; i < 8; ++i)
		pbFinalCount[i] = static_cast<unsigned char>((m_count[((i >= 4) ? 0 : 1)] >> ((3 - (i & 3)) * 8) ) & 0xFF); // Endian independent
	...
	for(i = 0; i < 20; ++i)
		m_digest[i] = static_cast<unsigned char>((m_state[i >> 2] >> ((3 -(i & 3)) * 8)) & 0xFF);
...
bool SHA1::getHash(unsigned char* pbDest20) const {
	if(pbDest20 == NULL)
	 return false;
